...
function GenerateScreen({ navigation, route }) {
            ...
            getSpotify('@spotify_token').then(tokenValue => {
                ...
                axios({
                    ...
                    for (let i = 0; i < route.params.exhibits.length; i++) {
                        ...
                        searchArtist(route.params.exhibits[i].name, tokenValue.access_token)
                    }
    ...
    function connectionTimeout() {
        setTimeout(() => {
            navigation.replace('Home', { alert: true, message: "CONNECTION TIMEOUT" })
        }, 7000)
    }
    ...
    function createPlaylist(userID, accessToken) {
        const data = JSON.stringify({
            name: "QR DOCENT",
            description: "A playlist of the exhibits you scanned with the QR DOCENT app.",
            public: "true"
        })
        axios({
            method: "POST",
    ...
    const _openSpotify = (playlistID) => {
        try {
            Linking.openURL("spotify:playlist:" + playlistID).catch((err) => {
                WebBrowser.openBrowserAsync(encodeURI("https://open.spotify.com/playlist/" + playlistID))
            }
            )
        } catch (err) {
            WebBrowser.openBrowserAsync(encodeURI("https://open.spotify.com/playlist/" + playlistID))
